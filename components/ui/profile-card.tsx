...
const ProfileCardComponent: React.FC<ProfileCardProps> = ({
  ...
  useEffect(() => {
    ...
    const handleClick = (): void => {
      ...
      if (anyMotion && typeof anyMotion.requestPermission === 'function') {
        anyMotion.requestPermission().then((state: string) => { if (state === 'granted') window.addEventListener('deviceorientation', onOrientation); }).catch(console.error);
  ...
  const shineStyle: React.CSSProperties = {
    maskImage: 'var(--icon)', maskMode: 'luminance' as const, maskRepeat: 'repeat', maskSize: '150%',

...
type DropzoneRootProps = ReturnType<typeof useDropzone>["getRootProps"];
...
export function FileUpload({ onChange, maxFiles, accept }: FileUploadProps) {
  const [files, setFiles] = useState<File[]>([]);
  const fileInputRef = useRef<HTMLInputElement>(null);
  const accumulatedRef = useRef<File[]>([]);

  const handleFileChange = (newFiles: File[]) => {
    const accumulated = [...accumulatedRef.current, ...newFiles].slice(0, maxFiles);
  ...
  return (
    <FileUploadDropzone getRootProps={getRootProps}>
      <FileUploadTrigger onClick={() => fileInputRef.current?.click()}>
        <HiddenFileInput
...
function FilePreviewList({ files }: { files: File[] }) {
  return (
    <>
      {files.map((file, index) => (
        <FilePreviewCard key={`${file.name}-${index}`} file={file} index={index} />
      ))}
    </>
  );
}

function FilePreviewCard({ file, index }: { file: File; index: number }) {
  return (
    <motion.div
...
function EmptyUploadState({ isDragActive }: { isDragActive: boolean }) {
  return (
    <>
      <motion.div
